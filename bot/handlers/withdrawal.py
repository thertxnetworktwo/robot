...
class WithdrawalHandler:
    ...
    async def withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user.balance < min_withdraw:
            await update.message.reply_text(f'❌ Minimum withdrawal amount is ${min_withdraw:.2f}\n\nYour balance: ${user.balance:.2f}')
            return
        ...
        if not trx_enabled and not ledger_enabled:
            await update.message.reply_text('❌ No withdrawal methods are currently available.')
            return
    ...
    async def handle_withdrawal_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if step == 'amount':
            ...
            if amount > max_withdraw:
                await update.message.reply_text(f'❌ Maximum withdrawal amount is ${max_withdraw:.2f}')
                return True
            ...
            if method == 'TRX':
                trx_enabled = db.get_setting('trx_withdrawal', 'on') == 'on'
                if not trx_enabled:
                    await update.message.reply_text('❌ TRX withdrawals are currently disabled.')
                    return True
                
                withdrawal['method'] = 'trx'
                withdrawal['step'] = 'wallet'
                await update.message.reply_text('Please send your TRX wallet address:')
                
            elif method == 'LEDGER':
                ledger_enabled = db.get_setting('ledger_withdrawal', 'on') == 'on'
                if not ledger_enabled:
                    await update.message.reply_text('❌ Ledger card withdrawals are currently disabled.')
                    return True
                
                withdrawal['method'] = 'ledger'
                withdrawal['step'] = 'wallet'
                await update.message.reply_text('Please send your Ledger card details:')
            else:
                await update.message.reply_text('❌ Invalid method. Please send TRX or LEDGER.')
